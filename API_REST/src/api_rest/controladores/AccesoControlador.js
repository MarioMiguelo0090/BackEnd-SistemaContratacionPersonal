...
export class AccesoControlador {
    ...
    try {
      ...
      if (!ResultadoValidacion.success) {
        return res.status(400).json({
          error: true,
          estado: 400,
          mensaje: ResultadoValidacion.error || "Datos inválidos",
        });
      }
      ...
      if (ResultadoLogin.estado !== 200) {
        return res.status(ResultadoLogin.estado).json({
          error: true,
      ...
      logger({ mensaje: error.message || error }); // Mejor registrar el mensaje
      res.status(500).json({
        error: true,
        estado: 500,
        mensaje: "Ha ocurrido un error en el servidor",
  ...
  ObtenerUsuarios = async (req, res) => {
    try {
      const ResultadoConsulta =
        await this.modeloAcceso.ObtenerTodosLosUsuarios();
      let resultadoConsulta = parseInt(ResultadoConsulta.estado);
      res.status(resultadoConsulta).json({
        error: resultadoConsulta !== 200,
        estado: resultadoConsulta,
